...
For shared HPC systems, do not run OCR, model inference, or batch extraction on a login node. Use the login node only for editing, file transfer, and job submission.

Example Slurm job:

```bash
#!/bin/bash
#SBATCH --job-name=chat_extract
...
#SBATCH --output=chat_extract_%j.out
#SBATCH --error=chat_extract_%j.err
...
export OLLAMA_MODELS=/var/local/storage/$USER/ollama_models
mkdir -p "$OLLAMA_MODELS"
mkdir -p results

ollama serve > results/ollama_${SLURM_JOB_ID}.log 2>&1 &
OLLAMA_PID=$!
...
```bash
tail -120 chat_extract_<job_id>.out
cat chat_extract_<job_id>.err
```
...
- remove case reports
- remove generated CSV transcripts if they contain sensitive content
- remove local paths, usernames, IP addresses, and cluster-specific details
- avoid committing model logs or debug folders that may contain extracted private text
